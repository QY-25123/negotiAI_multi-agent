...
export default function Negotiations() {
  ...
  return (
    <div className="p-8 max-w-5xl">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-[#1e293b]">Deals</h1>
        <p className="text-[#64748b] mt-1">Track all AI-powered deals</p>
      </div>
      {/* Filter tabs */}
        ...
        {FILTERS.map(f => (
          <button key={f.key} onClick={() => setStatusFilter(f.key)}
            className={`px-4 py-2 rounded-lg text-sm font-medium transition-all
              ${statusFilter === f.key ? "bg-[#ec4899] text-white" : "bg-white border border-[#fce7f3] text-[#64748b] hover:text-[#1e293b] hover:border-[#fbcfe8]"}`}>
            {f.label}
          </button>
        ))}
      </div>
      {isLoading ? (
        <div className="space-y-3">{[...Array(5)].map((_, i) => <div key={i} className="bg-white border border-[#fce7f3] rounded-xl h-24 animate-pulse" />)}</div>
      ) : (
        <div className="space-y-3">
          {(negotiations || []).length === 0 && <p className="text-center text-[#64748b] py-16">No deals found</p>}
          {(negotiations || []).map(neg => (
            <Link key={neg.id} href={`/negotiations/${neg.id}`}>
              <div className={`bg-white border rounded-xl p-5 hover:bg-[#fef3f8] transition-all cursor-pointer group
                ${neg.status === "active" ? "border-[#ec4899]/50" : "border-[#fce7f3] hover:border-[#fbcfe8]"}`}>
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    ...
                    <span className="text-[#fbcfe8] text-lg">→</span>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-sm font-semibold text-[#1e293b] group-hover:text-[#ec4899] truncate">{neg.title}</p>
